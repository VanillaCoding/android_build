...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^vu_") ; then
       VU_BUILD=$(echo -n $1 | sed -e 's/^vu_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $VU_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       VU_BUILD=
    fi
    export VU_BUILD
...
function print_lunch_menu()
{
    ...
    if [ "z${VU_DEVICES_ONLY}" != "z" ]; then
       echo "... and don't forget the f#cking Unicorns!"
    fi
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/vu/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the VU github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var VU_VERSION)
        ZIPFILE=vu-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.vu.device | grep -q "$VU_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $VU_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.vu.device | grep -q "$VU_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/vu/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
